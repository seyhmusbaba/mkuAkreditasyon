...
def status_by_threshold(pct: float, thresholds: Dict[str, float]) -> str:
    """
    thresholds örn:
      {"met": 70, "partially": 50}
    """
...
def compute(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for pid in pocs.keys():
        ...
        for did in docs.keys():
            ...
            if w > 0:
                den += w
                num += float(doc_stats.get(did, {}).get("success_pct", 0.0)) * w
                contrib_docs.append({"doc_id": did, "weight": w, "doc_pct": doc_stats.get(did, {}).get("success_pct", 0.0)})
    ...
    # --- PEA başarısı (doğrudan soru eşleştirmesi + PÖÇ katkısı)
    ...
    for aid in peas.keys():
        ...
        # 2. PÖÇ katkısından hesapla
        linked_pocs = [pid for pid, a_list in poc_pea_map.items() if aid in (a_list or [])]
        indirect_vals = [float(poc_stats.get(pid, {}).get("success_pct", 0.0)) for pid in linked_pocs]
        indirect_pct = statistics.mean(indirect_vals) if indirect_vals else 0.0
    ...
    narrative = {
        ...
        "doc_summary": [{"doc_id": did, "pct": st["success_pct"], "status": st["status"]} for did, st in doc_stats.items()],
        "poc_summary": [{"poc_id": pid, "pct": st["success_pct"], "status": st["status"]} for pid, st in poc_stats.items()],
